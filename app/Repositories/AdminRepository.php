...
class AdminRepository extends CoreRepository implements AdminInterface
{
    ...
    public function createOrUpdate($collection = [])
    {
        ...
        $update = [
            ...
            "id_role" => isset($collection['role_id']) ? $collection['role_id'] : 2,
            "is_requested" => isset($collection["is_requested"]) ? 1 : 0,
            "is_confirmed" => isset($collection["is_requested"]) ? 0 : 1,
            "id_shop" => isset($collection["shop_id"]) ? $collection["shop_id"] : null,
            "active" => isset($collection["active"]) ? $collection["active"] : 0,
        ];
        ...
        $admin = Model::updateOrCreate([
            "id" => isset($collection["id"]) ? $collection["id"] : 0
        ], $update);
        if ($admin) {
            $admin->balance()->create(['currency' => 'usd']);
        }
        return $this->successResponse("success", $admin);
    }
